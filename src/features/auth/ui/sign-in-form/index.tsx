...
export const SignInForm = () => {
  ...
  return (
    <form className="flex flex-col gap-5 w-full" onSubmit={signInFrom.handleSubmit}>
      {signInFrom.errorMessage && <div className="text-rose-500">{signInFrom.errorMessage}</div>}
      <UiTextField
        label="Номер телефона"
        inputProps={{
          placeholder: 'Введите номер телефона',
          ...signInFrom.register('phone', { required: true }),
        }}
      />
      <UiTextField
        label="Пароль"
        ...
        inputProps={{
          ...
          ...signInFrom.register('password', { required: true }),
        }}
        component={<ShowPasswordBtn isShow={showPassword.isShow} open={showPassword.open} close={showPassword.close} />}
      />
