...
export function FormCreateSale({ close, id }: { close: Function; id: string }) {
  ...
  const orgsOptions: UiSelectOption[] = data.orgs.data.map((item) => ({
    label: item.name,
    value: item.id,
  }))
  const billsOptions: UiSelectOption[] = data.bills.data.map((item) => ({
    label: item.name,
    value: item.id,
  }))
